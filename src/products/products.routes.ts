...
productRouter.get('/products', async (req: Request, res: Response) => {
	try {
		...
		if (!allProducts) return res.status(StatusCodes.NOT_FOUND).json({ error: `No products found!` })
...
productRouter.get("/product/:id", async (req: Request, res: Response) => {
	try {
		...
		if (!product) return res.status(StatusCodes.NOT_FOUND).json({ error: "Product does not exist" })
...
productRouter.post("/product", async (req: Request, res: Response) => {
	try {
		...
		if (!name || !price || !quantity || !image) return res.status(StatusCodes.BAD_REQUEST).json({ error: `Please provide all the required parameters..` })
...
productRouter.put("/product/:id", async (req: Request, res: Response) => {
	try {
		...
		if (!findProduct) return res.status(StatusCodes.NOT_FOUND).json({ error: `Product does not exist..` })
...
productRouter.delete("/product/:id", async (req: Request, res: Response) => {
	try {
		...
		if (!getProduct) return res.status(StatusCodes.NOT_FOUND).json({ error: `No product with ID ${req.params.id}` })
