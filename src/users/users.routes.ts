...
userRouter.post("/register", async (req: Request, res: Response) => {
	try {
		...
		if (!username || !email || !password) return res.status(StatusCodes.BAD_REQUEST).json({ error: `Please provide all the required parameters..` })
...
userRouter.post("/login", async (req: Request, res: Response) => {
	try {
		...
		const user = await database.findByEmail(email)
		if (!user) return res.status(StatusCodes.NOT_FOUND).json({ error: "No user exists with the email provided.." })
		const comparePassword = await database.comparePassword(email, password)
		if (!comparePassword) return res.status(StatusCodes.BAD_REQUEST).json({ error: `Incorrect Password!` })
...
userRouter.put('/user/:id', async (req: Request, res: Response) => {
	...
	try {
		...
		if (!username || !email || !password) return res.status(401).json({ error: `Please provide all the required parameters..` })
		if (!getUser) return res.status(404).json({ error: `No user with id ${req.params.id}` })
